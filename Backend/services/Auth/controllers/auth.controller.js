...
export const login = async (req, res) => {
  try {
    ...
    console.error("Login Error:", error); res.status(500).json({
      success: false, message: error.message, stack: error.stack, // Remove this in production 
    });
  }
}
...
export const logOut = async (req, res) => {
  try {
    ...
    //clearing the session from redis here when user logout
    const sessionId = req.cookies?.session
    await redis.del(`session-${sessionId}`)


    //clearing the session from cookie 
...
export const deductCredits = async (req,res)=>{
  try{
    ...
    if(user.credits<requiredCredits){
      return res.status(400).json({message:"Not enough credits. Please Upgrade Plan"})
    }
