...
export default function MessageBubble({ role, content, images }) {
  ...
  return (
    <div className={`flex ${isUser ? "justify-end" : "justify-start"}`}>
      <div
        className={`w-fit max-w-[92vw] md:max-w-[72%] px-4 py-2.5 rounded-2xl break-words ovverflow-hidden leading-relaxed
        ${
          isUser
          ...
          components={{
            ...
            a: ({ href, children }) => (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="text-indigo-400 underline break-all inline-flex items-center gap-1"
              >
                {children}
            ...
            code: ({ className, children }) => {
              ...
              return (
                <div className="my-4 overflow-hidden rounded-xl border border-white/10 bgg-[#1113118]">
                  <div className="flex items-center justify-between bg-[#1b1d24] border-b border-white/10 px-4 py-2">
                    <span className="uppercase text-ts text-slate-400">
                      {language}
                    </span>
                    <button
      ...
      {LightBox && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-6">
          ...
          <img
            src={LightBox}
            className="max-w-[90vw] max-h-[85vh] rounded-2xl border-white/10 shadow-2xl obejct-contain"
          />
        </div>
      )}
    </div>
